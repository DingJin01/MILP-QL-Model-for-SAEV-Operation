...
def run_scrolling_optimization(vehicles_file, orders_df, orders, D, A, T, params, charging_strategy, strategy):
    ...
    for start_hour in time_intervals:
        ...
        if strategy == 'milp_parallel':
            ...
            # Assuming vehicles, orders, D, A, T, PARAMS, and charging_strategy are defined
            print(vehicles)
            total_rewards, total_matched_orders, total_charging_events, agents = greedy_matching(
                vehicles, interval_orders, D, A, T, PARAMS, charging_strategy, interval_end_time = end_time
            )
            
            # Update vehicle_to_order_combination using total_matched_orders
            # vehicle_to_order_combination = {vehicle['id']: [] for vehicle in vehicles}  # Initialize if not already done
            for vehicle in vehicles:
                vehicle_id = vehicle['id']
                vehicle_to_order_combination[vehicle_id] = [order for order in total_matched_orders[vehicle_id]]
                print(f"Vehicle {vehicle_id} served orders: {vehicle_to_order_combination[vehicle_id]}")
            # Print total rewards
            print("Total Rewards from Greedy Strategy:", total_rewards)
...
if __name__ == "__main__":
    ...
    T = D / speed  # Assuming a constant speed of 20 units/hour

    # Update params with electricity price
    PARAMS['electricity_price'] = electricity_price

    # Define sample vehicles (with time as numeric hours)
    vehicles_file = 'vehicles_state.json'
    save_vehicle_state(VEHICLES, vehicles_file)  # Save initial vehicle state
    ...
    # Choose optimization strategy (e.g., 'greedy', 'milp_parallel', 'milp', 'ga')
    strategy = 'milp_parallel'
    charging_strategy = 'optimized'

    # Run the scrolling optimization
