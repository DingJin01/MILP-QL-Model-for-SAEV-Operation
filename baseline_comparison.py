...
def main():
    ...
    strategies = ['milp_parallel', 'nearest_neighbor']
    charging_strategies = ['optimized', 'no_optimization']

    # Run for each combination of strategy and charging strategy
    for strategy in strategies:
        for charging_strategy in charging_strategies:
            # Reset vehicle state before each run
            vehicles_file = 'vehicles_state.json'
            save_vehicle_state(VEHICLES, vehicles_file)

            print(f"\n=== Running {strategy} strategy with {charging_strategy} charging ===")
            run_scrolling_optimization(vehicles_file, orders_df, orders, D, A, D / PARAMS['vehicle_speed'], PARAMS, charging_strategy, strategy)
            clear_memory()  # Clear memory after each run
